        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 3rem;">
            ...
            <div>
                <img src="<?php echo htmlspecialchars($vehicle['image_path']); ?>"
                    ...
                    style="width: 100%; border-radius: 8px; box-shadow: var(--shadow); margin-bottom: 1.5rem;">
                <div style="background: #fff; padding: 1.5rem; border-radius: 8px; box-shadow: var(--shadow);">
                    <h3>Specifications</h3>
                    <ul style="list-style: none; margin-top: 1rem;">
                        <li style="margin-bottom: 0.5rem;"><strong>Brand:</strong>
                            <?php echo htmlspecialchars($vehicle['brand']); ?></li>
                        <li style="margin-bottom: 0.5rem;"><strong>Model Year:</strong>
                            <?php echo $vehicle['model_year']; ?></li>
                        <li style="margin-bottom: 0.5rem;"><strong>Type:</strong>
                            <?php echo ucfirst($vehicle['type']); ?></li>
                        <li style="margin-bottom: 0.5rem;"><strong>Terrain / Condition:</strong>
                            <?php echo ucfirst($vehicle['condition_type'] ?? 'city'); ?></li>
                        <li style="margin-bottom: 0.5rem;"><strong>Status:</strong>
                            <?php echo ucfirst($vehicle['status']); ?></li>
                    </ul>
                </div>
            </div>
            ...
            <div>
                <h1 style="color: var(--primary-blue); font-size: 2.5rem; margin-bottom: 0.5rem;">
                    <?php echo htmlspecialchars($vehicle['name']); ?></h1>
                <p style="font-size: 1.5rem; color: var(--primary-red); font-weight: 700; margin-bottom: 1.5rem;">Rs.
                    <?php echo $vehicle['price_per_day']; ?> <span
                        style="font-size: 1rem; color: var(--gray-text); font-weight: normal;">/ day</span></p>
                ...
                <div
                    style="background: #fff; padding: 2rem; border-radius: 8px; box-shadow: var(--shadow); border-top: 4px solid var(--primary-red);">
                    <h3 style="margin-bottom: 1.5rem;">Book This Vehicle</h3>

                    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                        <div class="alert alert-warning" style="margin-bottom: 1rem;">
                            <i class="fas fa-shield-alt"></i> You are viewing as Admin. Admins cannot make bookings.
                        </div>
                        <a href="manage_vehicles_ui.php" class="btn btn-blue-solid btn-block"
                            style="text-align: center;">Manage Vehicles</a>
                    <?php elseif (!$canBook): ?>
                        <div class="alert alert-danger" style="margin-bottom: 1rem;">
                            <i class="fas fa-exclamation-triangle"></i> <?php echo $bookError; ?>
                        </div>
                        <?php if (!isset($_SESSION['user_id'])): ?>
                            <a href="login.php" class="btn btn-primary btn-block" style="text-align: center;">Login Now to
                                Book</a>
                        <?php else: ?>
                            <!-- Inline Document Upload for Streamlined UX -->
                            <div
                                style="background: var(--light-bg); padding: 1.5rem; border-radius: 8px; border: 1px solid var(--border-color);">
                                <h4 style="margin-bottom: 0.5rem;">Upload Document Here</h4>
                                <p style="font-size: 0.85rem; margin-bottom: 1rem;">Admin will verify it before your booking is
                                    accepted.</p>
                                <form id="documentUploadForm" enctype="multipart/form-data">
                                    <div id="docAlert" class="alert" style="display:none; padding: 0.5rem; font-size: 0.85rem;">
                                    </div>
                                    ...
                                    <button type="submit" class="btn btn-outline btn-block" id="uploadBtn">Upload for
                                        Verification</button>
                                </form>
                            </div>
                        <?php endif; ?>
                    <?php elseif ($vehicle['status'] !== 'available'): ?>
                        <div class="alert alert-danger">
                            <i class="fas fa-ban"></i> This vehicle is currently <?php echo $vehicle['status']; ?> and
                            cannot be booked.
                        </div>
                    <?php else: ?>
                        <form id="bookingForm">
                            ...
                            <div class="form-group"
                                style="margin-bottom: 1.5rem; background: var(--light-bg); padding: 1rem; border-radius: 6px; border: 1px solid var(--border-color);">
                                <label
                                    style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer; margin: 0;">
                                    <input type="checkbox" name="with_driver" value="1" style="width: 20px; height: 20px;">
                                    <span style="font-weight: 700; color: var(--primary-blue);">Request Driver
                                        Inclusion</span>
                                    <span style="font-size: 0.8rem; color: var(--gray-text);">(Additional charges may
                                        apply)</span>
                                </label>
                            </div>
                            ...
                            <div
                                style="background: var(--light-bg); padding: 1rem; border-radius: 4px; border: 1px solid var(--border-color); margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-weight: 500;">Estimated Total:</span>
                                <span id="total_price_display"
                                    style="font-size: 1.2rem; font-weight: 700; color: var(--primary-blue);">Rs. 0.00</span>
                            </div>

                            <div class="alert alert-warning" style="margin-bottom: 1.5rem; font-size: 0.9rem;">
                                <strong><i class="fas fa-id-badge"></i> STRICT POLICY:</strong> You <strong>MUST</strong>
                                bring your original Citizenship and Driving License to the office physically before the keys
                                are handed over.
                            </div>

                            <button type="submit" class="btn btn-primary btn-block" id="bookBtn">Confirm Booking
                                Request</button>
                        </form>
                    <?php endif; ?>
                </div>
