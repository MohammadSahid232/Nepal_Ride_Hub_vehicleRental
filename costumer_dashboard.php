...
foreach ($documents as $doc) {
    if ($doc['document_type'] === 'citizenship' && $doc['status'] !== 'rejected')
        $hasCitizenship = true;
    if ($doc['document_type'] === 'license' && $doc['status'] !== 'rejected')
        $hasLicense = true;
}
?>
...
<section style="padding: 4rem 0;">
    <div class="container">
        ...
        <div class="alert alert-warning" style="margin-bottom: 2rem;">
            <i class="fas fa-exclamation-circle"></i> <strong>Important:</strong> Even with online verification, you
            <strong>MUST</strong> bring your original Citizenship/Passport and Driving License to our physical office to
            collect your vehicle. No exceptions.
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem;">
            ...
            <div
                ...
                <p style="font-size: 0.9rem; margin-bottom: 1rem;">To book a vehicle, verified citizenship and driving
                    license are required.</p>
                ...
                <?php else: ?>
                    <ul style="list-style: none;">
                        <?php foreach ($documents as $doc): ?>
                            <li
                                style="padding: 0.8rem; background: #f8f9fa; margin-bottom: 0.5rem; border-radius: 4px; display: flex; justify-content: space-between;">
            ...
            <div style="background: #fff; padding: 2rem; border-radius: 8px; box-shadow: var(--shadow);">
                ...
                <?php if (!$hasCitizenship || !$hasLicense): ?>
                    <div class="alert alert-danger">
                        <strong>Action Required:</strong> Please upload both your Citizenship and Driving License to enable
                        booking features.
                    </div>
                <?php endif; ?>
                                ...
                                <?php foreach ($bookings as $b): ?>
                                    <tr style="border-bottom: 1px solid var(--border-color);">
                                        <td style="padding: 1rem;">
                                            ...
                                            <small><?php echo ucfirst($b['vehicle_type']); ?> &bull;
                                                <?php echo ucfirst(str_replace('_', ' ', $b['purpose'] ?? 'travel')); ?>
                                                <?php if (isset($b['with_driver']) && $b['with_driver']): ?>
                                                    <br><span style="color:var(--primary-blue); font-weight:600;"><i
                                                            class="fas fa-user-tie"></i> With Driver</span>
                                                <?php endif; ?>
                                            </small>
                                        </td>
                                        <td style="padding: 1rem;">
                                        ...
                                        <td style="padding: 1rem;">
                                            <span
                                                style="padding: 0.3rem 0.6rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600; background: <?php echo $b['status'] === 'confirmed' ? '#d4edda' : '#fff3cd'; ?>; color: <?php echo $b['status'] === 'confirmed' ? '#155724' : '#856404'; ?>;">
